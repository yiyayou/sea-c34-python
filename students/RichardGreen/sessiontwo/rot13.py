...
'''converts a string to a list, splits each character up and'''
'''iterates it through a for loop converts its characters to rot13'''
'''encryption, accepts whitespace,and accepts punctuation '''

'''It will accept upper and lower case characters. It returns a rot13
 encrypted string'''
'''arguments:
x : a string
...
# load in a string
x = str('')
# Create a dictonary that handles the conversion/encryption of letters. Add a
# place at the end for whitespace
...
for id in split_list:
    ...
    if id in string.punctuation:
        results += "".join(id)
    # otherwise if its a whitespace or letter, lower or uppercase convert with
    # our dictonary above.
    else:
